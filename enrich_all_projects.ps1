...
foreach ($pdf in $pdfs) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  ✓ Successfully enriched $($pdf.Name)" -ForegroundColor Green
    } else {
        Write-Host "  ✗ Error enriching $($pdf.Name)" -ForegroundColor Red
    }
    Write-Host ""
}
...
Write-Host 'View JSON files:' -ForegroundColor Yellow
Write-Host '  Get-ChildItem data\enriched_*.json' -ForegroundColor Yellow
